...
def main():
    mdl_generalista = pickle.load(open('notebook/streamlit/modelos/RandomForestRegressor_generalista.sav','rb+'))
    mdl_premium1 = pickle.load(open('notebook/streamlit/modelos/RandomForestRegressor_premium_1.sav','rb+'))
    mdl_premium2 = pickle.load(open('notebook/streamlit/modelos/RandomForestRegressor_premium_2.sav','rb+'))
    mdl_premium3 = pickle.load(open('notebook/streamlit/modelos/BaggingRegressor_premium_3.sav','rb+'))
    mdl_premium4 = pickle.load(open('notebook/streamlit/modelos/RandomForestRegressor_premium_4.sav','rb+'))
    mdl_premium5 = pickle.load(open('notebook/streamlit/modelos/BaggingRegressor_premium_5.sav','rb+'))
    mdl_premium6 = pickle.load(open('notebook/streamlit/modelos/BaggingRegressor_premium_6.sav','rb+'))
    mdl_premium7 = pickle.load(open('notebook/streamlit/modelos/GradientBoostingRegressor_premium_7.sav','rb+'))
    ...
    model_selected = st.sidebar.selectbox('¿ Selecciona clasificador ?', option)
    ...
    if st.button('Valorizar'):        
        ...
        if model_selected == 'Premium 1':
            st.success(mdl_premium1.predict(df))
        if model_selected == 'Premium 2':
            st.success(mdl_premium2.predict(df))
        if model_selected == 'Premium 3':
            st.success(mdl_premium3.predict(df))
        if model_selected == 'Premium 4':
            st.success(mdl_premium4.predict(df))
        if model_selected == 'Premium 5':
            st.success(mdl_premium5.predict(df))
        if model_selected == 'Premium 6':
            st.success(mdl_premium6.predict(df))
        if model_selected == 'Premium 7':
            st.success(mdl_premium7.predict(df))
        if model_selected == 'Premium 8':
            st.success(mdl_premium8.predict(df))
